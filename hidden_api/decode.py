...
class decode:
    ...
    def initial_validation(self):
        if not self.content_type == "image/png":
            raise HTTPException(
                status_code=400,
                detail="Unsupported image format. Currently supported formats: image/png.",
            )
        ...
        try:
            for _ in range(0, number):
                result = (
                    result
                    + bin(
                        self.pixels[self.pixel_number[0], self.pixel_number[1]][
                            self.pixel_number[2]
                        ]
                    )[2:].zfill(8)[6:8]
                )
            ...
            raise HTTPException(
                status_code=400,
                detail="Input image doesn't appear to have any encoded message.",
            )
